...
export default function LoginPage() {
    ...
    const checkEmail = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setFlow(data.exists ? "signIn" : "signUp");
    ...
    const handleAuth = async (e: React.FormEvent) => {
        ...
        try {
            if (flow === "signUp") {
                ...
                // 1. Create Account
                const user = await account.create(ID.unique(), formData.email, formData.password);

                // 2. Login (create session) - Update profile with role if needed?
                // For now just basic auth.
                await account.createEmailPasswordSession(formData.email, formData.password);

                // Save role to prefs
                await account.updatePrefs({ role: isTutor ? "tutor" : "student" });
